...
const Pagination = ({
  ...
  const [perPagesOption, setPerPagesOption] = useState([10, 25, 50]);
  const [PerPage, setPerPages] = useState<number>(20);
  const [visablebtn, setvisablebtn] = useState<number>();
  ...
  useEffect(() => {
    setvisablebtn(pageSize/PerPage);
  }, []);

  const pages = () => {
    let elements = [];

    for (let index = 1; index <= pageSize / PerPage; index++) {
      elements.push(
        <li key={index}>
          <a
            href="#"
  ...
  return (
    <nav
      aria-label="Page navigation example bg-red-600 "
      className={visablebtn ? `flex items-center space-x-4 absolute bottom-0 left-0 right-0` : ` flex items-center space-x-4 absolute bottom-0 left-0 right-0 place-self-end`}
    >
            ...
            <a
              ...
              onClick={() =>
                setCurrentPage((prev: number) =>
                  prev != pageSize / PerPage ? prev + 1 : prev,
                )
              }
